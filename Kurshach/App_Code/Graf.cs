...
public class Graf<T>
{
    ...
    private List<pair> EdgesToFile = new List<pair>();
    ...
    public class MyFile
    {
        private StreamWriter write_text;
        ...
        public void WriteResultToFile(List<pair> p)
        {
            write_text = new StreamWriter(@"C:\inetpub\wwwroot\data.json", true);
            for (int i = 0; i < p.Count; i++)
            {
                if(i != p.Count - 1)
                    write_text.WriteLine("{\"src\": \"" + CityTA.GetCityNameByID(p[i].from).Trim() + 
                        "\", \"dest\": \"" + CityTA.GetCityNameByID(p[i].to).Trim() + "\"},");
                else
                    write_text.WriteLine("{\"src\": \"" + CityTA.GetCityNameByID(p[i].from).Trim() +
                        "\", \"dest\": \"" + CityTA.GetCityNameByID(p[i].to).Trim() + "\"}]}");
            }
    ...
    static MyFile muFilik = new MyFile();
    public virtual void Link(int from, int to, int cost, bool isPrima = false)
    {
            if (!ContainsLink(from, to))
    ...
    public static void Prima(Graf<T> graf)
    {
        ...
        while (vU.Count > 0)
        {
            ...
            EdgesPair = new pair(min.from, min.to);
            graf.EdgesToFile.Add(EdgesPair);
        ...
        muFilik.WriteResultToFile(graf.EdgesToFile);
            muFilik.Close();
    }
}