...
class Logger extends Component
{
     ...
     * `$categories` and `$excludePages` as parameters to retrieve the
     ...
     * @param array $excludePages list of categories that you want to exclude
     ...
     */
    public function getProfiling($categories = [], $excludePages = [])
    {
        $timings = $this->calculateTimings($this->messages);
        if (empty($categories) && empty($excludePages)) {
            return $timings;
        }
        ...
        foreach ($timings as $i => $timing) {
            ...
            if ($matched) {
                foreach ($excludePages as $category) {
                    $prefix = rtrim($category, '*');
                    foreach ($timings as $i => $timing) {
                        if (strpos($timing['category'], $prefix) === 0 && ($timing['category'] === $category || $prefix !== $category)) {
                            $matched = false;
