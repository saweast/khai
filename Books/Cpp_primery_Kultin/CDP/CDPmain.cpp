...
Graphics::TBitmap *bmPlay_d; // недоступная Play
    ...
    : TForm(Owner)
{
    ...
    bmPlay_d = new Graphics::TBitmap();
    bmStop   = new Graphics::TBitmap();

    // Загрузить картинки для кнопки Play/Stop.
    /* Загрузить картинку можно из файла, например так:
       bmPlay->LoadFromFile("play_s.bmp");
       bmStop->LoadFromFile("stop_s.bmp");
       но мы загрузим из ресурса. */
    bmPlay->LoadFromResourceID((int)HInstance,101);
    bmStop->LoadFromResourceID((int)HInstance,102);

    // отобразить на кнопке картинку "Play"
...
void __fastcall TForm1::MediaPlayerNotify(TObject *Sender)
{
  switch ( MediaPlayer->Mode )
  {
    case mpOpen: // пользователь открыл дисковод
    {
        ...
        SpeedButton2->Glyph->Assign(bmPlay_d);
        SpeedButton2->Tag  =  0;
